...
SubCommand* SubCommandHolderImpl::AddSubCommandToGroup(
  ...
  cmd_ptr->GetHolder()->SetListener(portability::make_unique<ListenerImpl>(this));
...
class ArgumentBuilderImpl : public ArgumentBuilder {
 public:
  ...
  void SetMetaVar(std::string val) override {
    meta_var_ = portability::make_unique<std::string>(std::move(val));
  }
...
std::unique_ptr<ArgumentController> ArgumentController::Create() {
  return portability::make_unique<ArgumentControllerImpl>();
}
...
std::unique_ptr<char[]> StringView::ToCharArray() const {
  auto dest = portability::make_unique<char[]>(size() + 1);
